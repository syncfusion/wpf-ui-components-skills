...
```
                        ...
                        Resizing, opacity and skewing examples are centralized in `visual-effects.md` to avoid duplication.

                        See: [visual-effects.md](visual-effects.md) for `ScaleFraction`, `OpacityFraction`, skew properties, and fraction-collection examples (per-position scaling/opacity/skew for CustomPath).
