...
- [Overriding Selection Behavior](#overriding-selection-behavior)
...
## Overriding Selection Behavior

Customize selection logic by creating a custom ComboBoxAdv class and overriding `OnItemChecked` and `OnItemUnchecked`.

### Use Case: Auto-Select Related Items

When user selects "Asia", automatically select "India" and "China".
...
{
    protected override ObservableCollection<object> OnItemChecked(
        object checkedItem, 
        ObservableCollection<object> selectedItems)
    {
        var item = ((FrameworkElement)checkedItem).DataContext;
        
        // If Asia is selected, add India and China
        if (item == this.Items[0])
            AddItems(selectedItems, new int[] { 1, 2 });
        
        // If Europe is selected, add France and Germany
        if (item == this.Items[4])
            AddItems(selectedItems, new int[] { 5, 6 });

        return base.OnItemChecked(checkedItem, selectedItems);
    }

    protected override ObservableCollection<object> OnItemUnchecked(
        object uncheckedItem, 
        ObservableCollection<object> selectedItems)
    {
        var item = ((FrameworkElement)uncheckedItem).DataContext;
        
        // If Asia is deselected, remove India and China
        if (item == this.Items[0])
            RemoveItems(selectedItems, new int[] { 1, 2 });
        
        // If Europe is deselected, remove France and Germany
        if (item == this.Items[4])
            RemoveItems(selectedItems, new int[] { 5, 6 });

        return base.OnItemUnchecked(uncheckedItem, selectedItems);
    }

    private void AddItems(ObservableCollection<object> selectedItems, int[] indices)
    {
    ...
    private void RemoveItems(ObservableCollection<object> selectedItems, int[] indices)
    {
        foreach (int i in indices)
        {
            if (selectedItems.Contains(this.Items[i]))
...
<Window xmlns:local="clr-namespace:YourNamespace">
    <Grid>
        <local:ComboBoxExt Width="250" Height="24"
                           AllowMultiSelect="True"
                           AllowSelectAll="True"
                           EnableOKCancel="True"
