...
### Create Float Window on Demand

```csharp
public void CreateFloatingToolWindow(string title, UIElement content)
{
    ContentControl floatWindow = new ContentControl();
    floatWindow.Content = content;
    
    DockingManager.SetHeader(floatWindow, title);
    DockingManager.SetState(floatWindow, DockState.Float);
    ...
    DockingManager.SetFloatingWindowRect(floatWindow, floatRect);
    
    dockManager.Children.Add(floatWindow);
}
```

### Multiple Float Windows

```xml
<syncfusion:DockingManager x:Name="dockManager"
                          UseNativeFloatWindow="True"
                          ShowInTaskbar="True">
    <ContentControl syncfusion:DockingManager.Header="Float 1"
                    syncfusion:DockingManager.State="Float"
                    syncfusion:DockingManager.FloatingWindowRect="100,100,400,300" />
    
    <ContentControl syncfusion:DockingManager.Header="Float 2"
                    syncfusion:DockingManager.State="Float"
                    syncfusion:DockingManager.FloatingWindowRect="550,100,400,300" />
    
    <ContentControl syncfusion:DockingManager.Header="Float 3"
                    syncfusion:DockingManager.State="Float"
                    syncfusion:DockingManager.FloatingWindowRect="100,450,400,300" />
</syncfusion:DockingManager>
```

### Remember Float Window Positions

```csharp
// Save positions before closing
public void SaveFloatWindowPositions()
{
    foreach (var child in dockManager.Children.OfType<FrameworkElement>())
    {
        if (DockingManager.GetState(child) == DockState.Float)
        {
            Rect rect = DockingManager.GetFloatingWindowRect(child);
            string header = DockingManager.GetHeader(child).ToString();
            
            // Save to settings
            Properties.Settings.Default[$"FloatPos_{header}"] = 
...
public void RestoreFloatWindowPositions()
{
    foreach (var child in dockManager.Children.OfType<FrameworkElement>())
    {
        string header = DockingManager.GetHeader(child).ToString();
        string savedPos = Properties.Settings.Default[$"FloatPos_{header}"] as string;
        ...
        if (!string.IsNullOrEmpty(savedPos))
        {
            ...
            if (parts.Length == 4)
            {
                ...
                DockingManager.SetFloatingWindowRect(child, rect);
            }
        }
    }
}
...
```csharp
// Handle float window state changes
dockManager.WindowStateChanged += (s, e) =>
{
    if (DockingManager.GetState(e.TargetElement) == DockState.Float)
    {
        Console.WriteLine($"{DockingManager.GetHeader(e.TargetElement)} is now floating");
        
        // Get the native window
        var floatWindow = DockingManager.GetFloatWindow(e.TargetElement);
        if (floatWindow is Window nativeWindow)
        {
            nativeWindow.Icon = new BitmapImage(new Uri("pack://application:,,,/icon.ico"));
        }
    }
};
```
